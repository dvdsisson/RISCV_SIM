...
void dpe(uint32_t kern, uint32_t* buff, uint8_t* out){
    ...
    out[0] = (uint8_t)((elem2*kern0) + (elem3*kern1) + (elem6*kern2) + (elem7*kern3));
    out[1] = (uint8_t)((elem1*kern0) + (elem2*kern1) + (elem5*kern2) + (elem6*kern3));
    out[2] = (uint8_t)((elem0*kern0) + (elem1*kern1) + (elem4*kern2) + (elem5*kern3));
    out[3] = (uint8_t)((elem6*kern0) + (elem7*kern1) + (elem10*kern2) + (elem11*kern3));
    out[4] = (uint8_t)((elem5*kern0) + (elem6*kern1) + (elem9*kern2) + (elem10*kern3));
    out[5] = (uint8_t)((elem4*kern0) + (elem5*kern1) + (elem8*kern2) + (elem9*kern3));
    out[6] = (uint8_t)((elem10*kern0) + (elem11*kern1) + (elem14*kern2) + (elem15*kern3));
    out[7] = (uint8_t)((elem9*kern0)  + (elem10*kern1) + (elem13*kern2) + (elem14*kern3));
    out[8] = (uint8_t)((elem8*kern0)  + (elem9*kern1)  + (elem12*kern2) + (elem13*kern3));
...
void dot_product(uint32_t start, uint32_t state, uint32_t* next_state){
    ...
    // State 10 (dot product)
        ...
        printf("Out_Layers: %x %x %x %x %x %x %x %x %x\n", out_layers[0], out_layers[1], out_layers[2], out_layers[3], out_layers[4], out_layers[5], out_layers[6], out_layers[7], out_layers[8]);
        printf("Vert: %d, Lat: %d\n", vert, lat);
        uint32_t* address[9];
        address[0] = PS.S1_OUTPUT_PTR + ((PS.S1_COLS - 1) * vert) + lat;
        address[1] = PS.S1_OUTPUT_PTR + ((PS.S1_COLS - 1) * vert) + lat + 1;
        address[2] = PS.S1_OUTPUT_PTR + ((PS.S1_COLS - 1) * vert) + lat + 2;
        address[3] = PS.S1_OUTPUT_PTR + ((PS.S1_COLS - 1) * (vert + 1)) + lat;
        address[4] = PS.S1_OUTPUT_PTR + ((PS.S1_COLS - 1) * (vert + 1)) + lat + 1;
        address[5] = PS.S1_OUTPUT_PTR + ((PS.S1_COLS - 1) * (vert + 1)) + lat + 2;
        address[6] = PS.S1_OUTPUT_PTR + ((PS.S1_COLS - 1) * (vert + 2)) + lat;
        address[7] = PS.S1_OUTPUT_PTR + ((PS.S1_COLS - 1) * (vert + 2)) + lat + 1;
        address[8] = PS.S1_OUTPUT_PTR + ((PS.S1_COLS - 1) * (vert + 2)) + lat + 2;
        printf("Address0: %x, %x\n", out_layers[0], address[0]);
        printf("Address1: %x, %x\n", out_layers[1], address[1]);
        printf("Address2: %x, %x\n", out_layers[2], address[2]);
        printf("Address3: %x, %x\n", out_layers[3], address[3]);
        printf("Address4: %x, %x\n", out_layers[4], address[4]);
        printf("Address5: %x, %x\n", out_layers[5], address[5]);
        printf("Address6: %x, %x\n", out_layers[6], address[6]);
        printf("Address7: %x, %x\n", out_layers[7], address[7]);
        printf("Address8: %x, %x\n", out_layers[8], address[8]);
        *next_state = 11;
    }
